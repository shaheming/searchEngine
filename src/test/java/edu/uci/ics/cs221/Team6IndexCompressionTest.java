...
public class Team6IndexCompressionTest {
   ...
   */
  private InvertedIndexManager createPositionalManager(
      String path, Analyzer analyzer, boolean ifCompress) {
    File directory = new File(path);
    if (!directory.exists()) {
      directory.mkdirs();
    }
    if (ifCompress)
      return InvertedIndexManager.createOrOpenPositional(
          path, analyzer, new DeltaVarLenCompressor());
    else return InvertedIndexManager.createOrOpenPositional(path, analyzer, new NaiveCompressor());
  }

  /**
   * input documents 10 times into each manager, compare the reader counters We assume the
   * compressed one will be 2/3 smaller than non-compressed one
   */
  @Test
  public void test1() {
    // naive compressor manager add document
    for (int i = 0; i < 100; i++) nonCompressManager.addDocument(sampleDoc);
    nonCompressManager.flush();
    nonCompressWriteCounter = PageFileChannel.writeCounter;
    PageFileChannel.resetCounters();

    // Compress manager add documents
    for (int i = 0; i < 100; i++) compressManager.addDocument(sampleDoc);
    compressManager.flush();
    compressWriteCounter = PageFileChannel.writeCounter;
    PageFileChannel.resetCounters();

    assertEquals(true, compressWriteCounter / nonCompressWriteCounter < 2.0 / 3);
  }

  /**
   * search the phase and compare read counters We assume the compressed one will be 2/3 smaller
   * than non-compressed one
   */
  @Test
  public void test2() {
    ...
    for (int i = 0; i < 10; i++) nonCompressManager.searchPhraseQuery(keywords);
    nonCompressReadCounter = PageFileChannel.readCounter;
    PageFileChannel.resetCounters();

    for (int i = 0; i < 10; i++) compressManager.searchPhraseQuery(keywords);
    compressReadCounter = PageFileChannel.readCounter;
    PageFileChannel.resetCounters();

    assertEquals(true, compressReadCounter / nonCompressReadCounter < 2.0 / 3);
  }
