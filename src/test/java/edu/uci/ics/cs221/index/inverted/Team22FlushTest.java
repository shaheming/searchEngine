...
public class Team22FlushTest {
  ...
  public void verifyContextTest() throws Exception {
    ...
    analyzer =
        new ComposableAnalyzer(
            tokenizer, stemmer); // create composable analyzer for documents tokenization
    ...
    Document doc4 =
        new Document(
            "Four basic operations in the effective"
    ...
    for (Document d : docs) {
      ...
      for (String token : tokens) {
        if (!output.containsKey(
            token)) { // if the output map doesn't contain this token, create new entry
