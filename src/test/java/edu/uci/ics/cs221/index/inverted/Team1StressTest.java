...
public class Team1StressTest {
  ...
  private Document[] largeDocs = new Document[] {generateDoc(5), generateDoc(5)};

  private Document[] manyDocs =
      new Document[] {
        generateDoc(1),
        generateDoc(1),
        generateDoc(1),
  ...
  public void test1() throws Exception {
    ...
    assertEquals(largeDocs.length, docs.size());
  ...
  @Test(timeout = 300000)
  public void test2() {
    for (Document doc : manyDocs) iim.addDocument(doc);
  ...
  private Document generateDoc(int n) {
    try {
      URL dictResource =
          Team1StressTest.class.getClassLoader().getResource("cs221_frequency_dictionary_en.txt");
      String text =
          Files.readAllLines(Paths.get(dictResource.toURI())).stream()
